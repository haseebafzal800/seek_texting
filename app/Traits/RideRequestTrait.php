...
trait RideRequestTrait {
    ...
    public function acceptDeclinedRideRequest($ride_request,$request_data = null)
    {
        ...
        $radius = Setting::where('type','DISTANCE')
                 ->where('key','DISTANCE_RADIUS')
                 ->pluck('value')
                 ->first() ?? 50;            
        ...
        $nearby_driver = User::selectRaw("id, user_type, player_id, latitude, longitude, 
                        ( $unit_value * acos( cos( radians($latitude) ) * cos( radians( latitude ) ) * 
