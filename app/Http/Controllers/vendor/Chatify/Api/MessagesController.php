...
class MessagesController extends Controller
{
    ...
    public function fetch(Request $request)
    {
        ...
        $query = Chatify::fetchMessagesQuery($request['id'])->orderBy('created_at','ASC')->latest();
    ...
    public function seen(Request $request)
    {
        ...
        return Response::json([
            'status' => true,
            'message' => "Messages Seen Status changed"
        ], 200);
    }
    ...
    // }
    {
        ...
        if (count($usersList) > 0) {
            ...
            foreach ($usersList as $user) {
                $data = Chatify::getContactItem($user);
              $v = json_encode($data);
              $st = json_decode($v);
      
                $contacts[]= $st->original;
            }
        } else {
            $contacts = [];
        }
    ...
    {
        // delete
        Chatify::deleteConversation($request['to_id']);
