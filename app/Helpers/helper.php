...
function sendSMS($cam_data = array())
{

    // twilioSMS(1,1,'');
    $sendToEmailsList = array();
    $notSendToEmails = array();
    $sendToEmails = array();
    $list_id = $cam_data->campaign_send_to_list_ids;
    if ($cam_data->campaign_send_to_emails) {
        $sendToEmails = explode(',', $cam_data->campaign_send_to_emails);
    }
    if ($cam_data->campaign_not_send_to_emails) {
        $notSendToEmails = explode(',', $cam_data->campaign_not_send_to_emails);
    }
    $sendToEmailsListObj = Contactlist::where(['list_id' => $cam_data->campaign_send_to_list_ids, 'status' => 'active'])->get();
    if ($sendToEmailsListObj) {
        foreach ($sendToEmailsListObj as $ste_key) {
            $sendToEmailsList[] = $ste_key->contact;
        }
    }
    $sendToArray = Arr::collapse([$sendToEmailsList], [$sendToEmails]);
    $sendToEmailsFinal = array_diff($sendToArray, $notSendToEmails);
    // var_dump($sendToEmailsFinal); die; //drop sub array from super array
    $user = User::where('id', $cam_data->user_id)->first();
    ...
    $sentCount = 0;
    $sleepCounter = 0;
    $counterToAddSmsStatusJob = 0;
    foreach ($sendToEmailsFinal as $l_key) {
        $recPh = formatNumber($l_key);

        $where = 'user_id = ' . $user->id . ' AND (contact = ' . $recPh . ')';
        $receiver = Contactlist::whereRaw($where)->first();
        
        // $receiver = Contactlist::where(['contact' => $l_key, 'list_id' => $list_id])->first();
        //  return $user;
        $sc_date = date('Y-m-d');
        ...
        if ($sentCount >= $user->daily_text_limit) {
            broadcast(new CampaignNotificationEvent($cam_data, 'campaignFailNotification-' . $cam_data->user_id, 'cf'))->toOthers();
            return 'limit';
            break;
        } else {
            $smsLength = Str::length($cam_data->message);
            ...
            if(isset($receiver->contact) && isset($user->contact_number)){
            $resp = twilioSMS($receiver->contact, $user->contact_number, $cam_data->message);
            //  return $resp;
            if ($resp) {
                // if ($counterToAddSmsStatusJob == 0) {
                //     dispatch(new SmsStatusJob($user->contact_number, date('Y-m-d')));
                // }
                // $counterToAddSmsStatusJob++;
                ...
                if ($y == 0) {
                    ...
                    $dataChat = array(
                        ...
                        'resent_sent' => date('Y-m-d H:i:s'),
                        'updated_at' => date('Y-m-d H:i:s'),
                    );
                    // var_dump($dataChat);exit;
                    $v =    Chat::create($dataChat);
                ...
                $data = array(
                    'from_id' => $user->id,
                    'type' => 'user', //user means our user and subscriber means 
                    'to_id' => $receiver->id,
                    'message' => $cam_data->message,
                    'created_at' => date('Y-m-d H:i:s'),
                    'msg_sid' => $resp->sid,
                    'status' => $resp->status,
                    'campaign_id' => $cam_data->campaigns_id,
                    'chat_id' => $chat_id,
                );
                ChMessage::insert($data);
            } else {
                continue;
            }
            unset($resp);
    ...
    DB::table('campaign_details')->where('id', $cam_data->id)->update($statusData);
    DB::table('campaigns')->where('id', $cam_data->campaigns_id)->update($statusData);
    // \Artisan::call('cache:clear');
    dispatch(new SmsStatusJob($user->contact_number, date('Y-m-d'), $cam_data->campaigns_id));
    return 'Success! campaign created and run';
}
...
function sendSMSBeforeSorting($cam_data = array())
{
...
function twilioVoiceCalls($from = null, $to = null)
{
    ...
    try {
        // 'messagingServiceSid' => 'MG3894f8030043d170a93749401045a6ab', 
        $resp = $client->account->calls->create(
            $to_number,
            $twilio_number,
            array(
                "url" => "https://master.d2w4rhm9txhteo.amplifyapp.com/voice-call"
            )
        );
        return $resp->sid;
    } catch (Exception $e) {
...
function twilioSMS($receiver, $sender, $message)
{
    $account_sid = getenv("TWILIO_SID");
    $auth_token = getenv("TWILIO_TOKEN");
    // $sender = getenv("TWILIO_FROM");
    // $receiver = "+13477194581";
    $callbackUrl = getenv("SMS_STATUS_CALLBACK_URL");
    // $callbackUrl = "https://api.seektexting.com/webhook/sms_status";

    $client = new Client($account_sid, $auth_token);

    /*$resp = $client->messages->create($receiver, [
        'from' => $sender,
        'body' => $message,
...
function twilioSMSIndividual($receiver, $sender, $message)
{
    $account_sid = getenv("TWILIO_SID");
    $auth_token = getenv("TWILIO_TOKEN");
    // $sender = getenv("TWILIO_FROM");
...
function validate_mobile($mobile)
{
    ...
    try {
        $phone_number = $twilio->lookups->v2->phoneNumbers($mobile)->fetch();
        // $phone_number = $phoneApi->fetch();

        return $phone_number->valid ? $phone_number->phoneNumber : $phone_number->valid;
    } catch (Exception $e) {
        return false;
    }
}
function dateBeforeOneMonth()
{

    // Get the date 30 days ago from today
    $date = Carbon::now()->subDays(30);

    // Get the year and date in your desired format
    $yearAndDate = $date->format('Y-m-d') . ' 00:00:00';

    // Print out the year and date
    return $yearAndDate;
    // return $daysInMonth;
}
// function get
...
{
    if (!$zip || is_null($zip) || empty($zip))
        return null;
    $gkey = getenv('GOOGLE_MAP_KEY');
    $url = "https://maps.googleapis.com/maps/api/geocode/json?address=" . trim($zip) . "&sensor=true&key=$gkey";
